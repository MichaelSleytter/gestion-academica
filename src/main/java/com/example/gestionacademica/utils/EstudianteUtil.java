package com.example.gestionacademica.utils;
...
public final class EstudianteUtil {
    ...
    public static String generarCodigoDesdeId(long id, int ancho) {
        ...
        if (ancho <= 0) throw new IllegalArgumentException(
            "ancho debe ser > 0"
        );
    ...
    public static String generarCodigoAleatorio(int digitos) {
        if (digitos <= 0 || digitos > 18) throw new IllegalArgumentException(
        ...
        return (
            PREFIJO_ESTUDIANTE + "-" + String.format(Locale.ROOT, fmt, valor)
        );
    }
    ...
    public static String generarContrasenaSegura(int longitud) {
        if (longitud < 8) throw new IllegalArgumentException(
            "longitud minima 8"
        );
        String chars =
